...
export const addLegalCaseController = asyncHandler(
    async (req, res):Promise<any>  => {
        ...
        const {
            ...
            applicantId, respondentId, assignedToId, provinceId,
            filingDate, hearingDates, notes
        } = req.body;

        // Ensure mandatory fields are present
        if (!title || !category) {
            return res.status(400).json(new ApiResponse(false, 400, 'Title and Category are required.', null));
        }
        ...
        if (req.files && Array.isArray(req.files) && req.files.length > 0) {
            const uploadPromises = req.files.map(file => uploadToCloudinary(file.path, 'legal-documents'));
            const uploadedDocuments = await Promise.all(uploadPromises);
            
            // Collect the secure URLs from the upload response
            documents = uploadedDocuments.filter(url => url).map(upload => upload.secure_url || upload);
        }
        ...
        return res.status(201).json(new ApiResponse(true, 201, 'Legal Case created successfully.', newCase));
    }
);

// --- 2. FETCH LegalCases (List and Filter) ---
export const fetchLegalCasesController = asyncHandler(
    async (req, res):Promise<any>  => {
        // @ts-ignore
...
//  @ts-ignore
        ...
        return res.status(200).json(new ApiResponse(true, 200, 'Legal Cases fetched successfully.', {
            cases,
            //  @ts-ignore
...
export const getLegalCaseByIdController = asyncHandler(
    async (req, res):Promise<any>  => {
...
export const editLegalCaseController = asyncHandler(
    async (req, res):Promise<any>  => {
        const { caseId } = req.params;
        ...
        return res.status(200).json(new ApiResponse(true, 200, 'Legal Case updated successfully.', updatedCase));
    }
);

// --- 5. CHANGE LegalCase Status ---
export const changeLegalCaseStatusController = asyncHandler(
    async (req, res):Promise<any> => {
        const { caseId } = req.params;
        ...
        if (!Object.values(LegalCaseStatus).includes(newStatus)) {
            return res.status(400).json(new ApiResponse(false, 400, `Invalid status value. Must be one of: ${Object.values(LegalCaseStatus).join(', ')}.`, null));
        }
        ...
        return res.status(200).json(new ApiResponse(true, 200, `Status changed to ${newStatus}.`, updatedCase));
    }
);

// --- 6. DELETE LegalCase ---
export const deleteLegalCaseController = asyncHandler(
 async (req, res):Promise<any> => {
        const { caseId } = req.params;
